...
def importar_funcionarios_excel():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            # Os dados começam na coluna B (índice 1) porque a coluna A está vazia
            ...
            # Chefia costuma estar nas colunas J e K (índices 10 e 11 se houver deslocamento)
            chefia_nome = str(row[10]).strip() if len(row) > 10 and row[10] else ""
            chefia_matricula = str(row[11]).strip() if len(row) > 11 and row[11] else ""
            ...
            if nome_setor not in setores_cache:
                ...
                novo_setor_ref.set({
                    ...
                    'chefia_matricula': chefia_matricula
                })
